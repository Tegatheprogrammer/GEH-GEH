...
function downloadCurrentSong() {
  const songUrl = mainAudio.src; // Get the current song's source
  if (songUrl) {
    const anchor = document.createElement("a");
    anchor.href = songUrl;
    anchor.download = `${musicName.innerText} - ${musicArtist.innerText}.mp3`; // Set file name as 'Song Name - Artist.mp3'
    document.body.appendChild(anchor);
    anchor.click();
    document.body.removeChild(anchor);
  } else {
    alert("No song is currently playing to download!");
  }
}